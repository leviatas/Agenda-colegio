...
import { DESDE, HASTA, fmtHora } from '../lib/agenda';

// Sólo alta de un evento propio. Editarlo, borrarlo o compartirlo puntual se
// hace clickeándolo directo en el calendario o en "Próximas fechas"
// (EditEventDialog.jsx); compartir TODOS los eventos vive en el ícono de
// compartir al lado de la cuenta, en Masthead.jsx (CompartirTodoDialog.jsx).
// Este modal no necesita saber nada de eso.
const VACIO = { title: '', date: '', endDate: '', time: '', endTime: '' };

function Cuerpo({ onClose }) {
  const { user } = useAuth();
  const { agregarMio } = useEventos();
  ...
  return (
    <>
      <div className="modal-head">
        <h2 id="adder-title">Agregar evento</h2>
      </div>
